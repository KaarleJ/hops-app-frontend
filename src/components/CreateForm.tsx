...
import { Course, NewCourse, Period, PeriodOption, YearOption } from "../types";
...
const yearToOptions = (year: number): YearOption[] => {
  ...
  options.sort()
  return options.map((option) => ({value: option, label:(option-1).toString() + '-' + option.toString()}));
}
...
const validationSchema = yup.object().shape({
  ...
  ects: yup
  .string()
  .required('ects missing'),
  year: yup
  .string()
  .required('year missing'),
  startPeriod: yup
  .string()
  .required('Startperiod missing'),
  endPeriod: yup
  .string()
  .required('Endperiod missing')
})
...
const CourseForm = ({ onSubmit, error }: formProps) => {
  return (
    <Box sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 5}}>
      ...
      <Typography variant='h4' sx={{ padding: 5, color: 'black' }}> Add a course </Typography>
      
      <Typography variant='h6' color='text'> Course name </Typography>

      <FormikTextInput name='name' placeholder='course name here' sx={{ mb: 2, mt: 4}}/>

      <Typography variant='h6' color='text'> Course code </Typography>

      <FormikTextInput name='code' placeholder='Course code here' sx={{ mb: 2, mt: 2}}/>

      <Typography variant='h6' color='text'> Ects </Typography>

      <FormikTextInput name='ects' placeholder='Course ects here' sx={{ mb: 2, mt: 2}}/>

      <SelectField label='year' name='year' options={yearOptions} />

      <SelectField label='startperiod' name='startPeriod' options={periodOptions} />
...
const CreateForm = ({ handleCourseUpdate}: CreateFormProps) => {
  ...
  const onSubmit = async (values: NewCourse) => {
    ...
    try {
      const course = await create({ name, code,  ects: Number(ects), year, startPeriod, endPeriod});
      if (course) {
  ...
  return (
    <Formik initialValues={initialValues} onSubmit={onSubmit} validationSchema={validationSchema}>
      {({handleSubmit}) => <CourseForm onSubmit={handleSubmit} error={error}/>}
    </Formik>
  );
};
